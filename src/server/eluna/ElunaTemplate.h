...
struct ElunaRegister
{
    ...
    ElunaRegister(const char* name, int(*func)(Eluna*), MethodRegisterState state = METHOD_REG_ALL, uint32 flags = METHOD_FLAG_NONE)
        : name(name), mfunc(reinterpret_cast<typename std::conditional<std::is_same_v<T, void>, int(*)(Eluna*), int(*)(Eluna*, T*)>::type>(func)), regState(state), flags(static_cast<MethodFlags>(flags)) {}
