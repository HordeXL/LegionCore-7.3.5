 ...
 */
template<typename T>
inline bool LuaRetIs(Eluna* E, lua_State* L, int idx) { return LuaRet<T>::Is(L, idx); }
template<typename T>
inline auto LuaRetGet(Eluna* E, lua_State* L, int idx) { return LuaRet<T>::Get(E, L, idx); }
...
{
    ( [&] {
        using T = typename std::remove_reference<typename std::tuple_element<Is, std::tuple<Outs&...> >::type>::type;
        const int idx = r + static_cast<int>(Is);

        if (LuaRetIs<T>(this, L, idx))
        {
            std::get<Is>(outs) = LuaRetGet<T>(this, L, idx);
