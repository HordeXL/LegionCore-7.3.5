    ...
    int GetQuest(Eluna* /*E*/)
{
    ...
    int GetPlayersInWorld(Eluna* E)
    {
        ...
        for (HashMapHolder<Player>::MapType::const_iterator it = m.begin(); it != m.end(); ++it)
        {
            if (Player* player = it->second)
            {
                ...
                if ((team == TEAM_NEUTRAL || uint32(player->GetTeamId()) == team) && (!onlyGM || player->GetSession()->GetSecurity() > 0))
                {
                    E->Push(player);
     ...
     */
    int RunCommand(Eluna* /*E*/) { return 1; }
    ...
    int WorldDBQueryAsync(Eluna* E)
    {
        ...
        E->GetQueryProcessor().AddQuery(WorldDatabase.AsyncQuery(query).WithCallback([E, funcRef](QueryResult result)
        {
    ...
    int CharDBQueryAsync(Eluna* E)
    {
        ...
        // Add an asynchronous query callback
        E->GetQueryProcessor().AddQuery(CharacterDatabase.AsyncQuery(query).WithCallback([E, funcRef](QueryResult result)
        {
            ElunaQuery* eq = result ? &result : nullptr;
    ...
    int AuthDBQueryAsync(Eluna* E)
    {
        ...
        // Add an asynchronous query callback
        E->GetQueryProcessor().AddQuery(LoginDatabase.AsyncQuery(query).WithCallback([E, funcRef](QueryResult result)
        {
            ElunaQuery* eq = result ? &result : nullptr;
    ...
    int HotfixDBQueryAsync(Eluna* E)
    {
        ...
        // Add an asynchronous query callback
        E->GetQueryProcessor().AddQuery(HotfixDatabase.AsyncQuery(query).WithCallback([E, funcRef](QueryResult result)
        {
            ElunaQuery* eq = result ? &result : nullptr;
     ...
     */
    int AddVendorItem(Eluna* /*E*/) { return 0; }
     ...
     * @param uint32 item : [Item] entry Id
     */
    int VendorRemoveItem(Eluna* /*E*/) { return 0; }
     ...
     * @param uint32 entry : [Creature] entry Id
     */
    int VendorRemoveAllItems(Eluna* /*E*/) { return 0; }
    ...
    int SendMail(Eluna* E)
    {
        ...
        /* CharacterDatabaseTransaction removed */
        uint8 addedItems = 0;
...
#else
            if (Item* item = Item::CreateItem(entry, amount, 0))
#endif
            {
                draft.AddItem(item);
