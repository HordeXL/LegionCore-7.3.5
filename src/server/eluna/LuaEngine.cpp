...
void Eluna::_ReloadEluna()
{
    ...
    // Cancel all pending async queries (not available in this core version)
    QueryCallbackProcessor().ProcessReadyQueries(); // flush any pending
#endif
...
#if defined ELUNA_TRINITY && ELUNA_EXPANSION >= EXP_RETAIL && defined(TRACKABLE_PTR_NAMESPACE)
