...
static bool AddBotToPlayerGroup(Player* owner, Player* bot)
{
    ...
    // 组队时把 bot 等级对齐到玩家，并触发 PlayerBotSetting 重置（自动重配装备/技能）
    if (PlayerBotSession* pSession = dynamic_cast<PlayerBotSession*>(bot->GetSession()))
    {
        pSession->RemoveScheduleByType(BotGlobleScheduleType::BGSType_Settting);
        BotGlobleSchedule schedule(BotGlobleScheduleType::BGSType_Settting, 0);
        schedule.parameter1 = owner->getLevel();
        schedule.parameter2 = owner->getLevel();
        schedule.parameter3 = bot->GetTalentType() + 1;
...
public:
    ...
    static int32 sMaxOnlineBeforeLogout;
    ...
    // GM：.bot login [数量] - 上线机器人到指定数量（无参数 = 恢复原上限）
    static bool HandleBotLoginCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            // 无参数：恢复 .bot logout 前的原上限（若之前没有 logout 则保持当前值）
            if (sMaxOnlineBeforeLogout > 0)
            {
                sPlayerBotMgr->SetMax(sMaxOnlineBeforeLogout);
                sMaxOnlineBeforeLogout = 0;
            }
        }
    ...
    // GM：.bot logout - 全部机器人下线（同时置上限为 0 阻止自动补满）
    static bool HandleBotLogoutCommand(ChatHandler* handler, char const* args)
    {
        if (sPlayerBotMgr->m_MaxOnlineBot > 0)
            sMaxOnlineBeforeLogout = sPlayerBotMgr->m_MaxOnlineBot;
        sPlayerBotMgr->SetMax(0);
        sPlayerBotMgr->AllPlayerBotLogout();
        handler->PSendSysMessage("已请求全部机器人下线（原上限 %d 已保存，.bot login 可恢复）", sMaxOnlineBeforeLogout);
        return true;
    }
    ...
    // GM：.bot max <数量> - 设置在线 bot 上限（写入配置文件持久生效）
    static bool HandleBotMaxCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (inFile)
        {
            ...
            std::string key = "PlayerBot.MaxOnlineCount";
            std::string newValue = std::to_string(n);

            std::regex optionRegex(key + R"(\s*=\s*\d+)");
            if (std::regex_search(content, optionRegex))
            {
                content = std::regex_replace(content, optionRegex, key + " = " + newValue);
        ...
        if (saved)
        {
            ...
            {
                // 重载后 SetMax 会保留，但保险起见重新应用
                sPlayerBotMgr->SetMax(sConfigMgr->GetIntDefault("PlayerBot.MaxOnlineCount", 10));
                handler->PSendSysMessage("已设置机器人上限：%d（已写入配置文件并生效）", sPlayerBotMgr->m_MaxOnlineBot);
            }
            else
            {
                handler->PSendSysMessage("已设置机器人上限：%d（已写入配置文件，但重载失败：%s）", n, error.c_str());
            }
        }
        else
        {
            handler->PSendSysMessage("已设置机器人上限：%d（仅本次运行生效，配置文件写入失败）", n);
        }
        return true;
    }
...
int32 bot_commandscript::sMaxOnlineBeforeLogout = 0;
