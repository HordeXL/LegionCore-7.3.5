...
bool PlayerBotSession::IsBotSession() const
{
	return true;
}

bool PlayerBotSession::HasBGSchedule() const
{
	for (auto itSc = m_Schedules.begin(); itSc != m_Schedules.end(); itSc++)
	{
...
bool PlayerBotSession::HasSchedules() const
{
	return m_Schedules.size() > 0;
}

bool PlayerBotSession::IsAccountBotSession() const
{
	return m_AccountBot;
}
