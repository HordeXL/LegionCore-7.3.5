...
std::string ToolCharaterInfo::SerializerInfo()
{
	std::string result;
	if (guid != 0)
	{
		result = std::to_string(guid) + "," + name + "," + std::to_string(race);
	}
	else
	{
		result = "0";
...
std::string ToolAccountInfo::SerializerInfo()
{
	WorldSession* pSession = sWorld->FindSession(id).get();
	std::string result;
	result = std::to_string(id) + "," + username + "," + std::to_string((pSession) ? uint32(pSession->GetSecurity()) : 0) + "," + online.SerializerInfo();
	return result;
}
