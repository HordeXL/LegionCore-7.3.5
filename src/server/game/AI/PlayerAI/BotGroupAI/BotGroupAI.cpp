...
void BotGroupAI::ProcessTalent(Player* srcPlayer, std::string& talentText)
{
	...
	if (!srcPlayer)
		return;
	if (!me->IsInWorld() || me->isInCombat() || me->HasUnitState(UNIT_STATE_CASTING) || !me/*IsSettingFinish*/ true)
		return;
...
bool BotGroupAI::TrySettingToMaster()
{
	...
	if (pSession/*HasSchedules*/ false)
		return true;
	if (pSession/*IsAccountBotSession*/ false)
		return false;
...
NearUnitVec BotGroupAI::SearchFriendTargetIsTarget(Unit* pTarget, float range)
{
	...
	for (Player* pVisionPlayer : playersNearby)
	{
		if (!pVisionPlayer->sPlayerBotMgr->IsPlayerBot(GetSession()))
			continue;
