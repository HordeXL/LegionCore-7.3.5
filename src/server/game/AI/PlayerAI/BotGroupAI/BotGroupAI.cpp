...

{
	...
	if (!me->IsInWorld() || me->isInCombat() || me->HasUnitState(UNIT_STATE_CASTING) || !true /*IsSettingFinish*/)
...

{
...

	{
		...
		if (player == NULL || (player->GetSession() && sPlayerBotMgr->IsPlayerBot(player->GetSession())))
...

{
...

	{
		...
		if (!sPlayerBotMgr->IsPlayerBot(pVisionPlayer->GetSession()))
