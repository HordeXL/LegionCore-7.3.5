...
SpellEntry* BotUtility::BuildNewArenaSpellEntry()
{
	...
	SpellEffectEntry* Effect[] ={ const_cast<SpellEffectEntry*>(newSpellEntry->GetSpellEffect(0)), const_cast<SpellEffectEntry*>(newSpellEntry->GetSpellEffect(1)), const_cast<SpellEffectEntry*>(newSpellEffectEntry->GetSpellEffect(2)) };
	Effect[0]->Effect = 6;
	Effect[1]->Effect = 6;
	Effect[2]->Effect = 6;
...
void BotUtility::ProcessGroupTankPullTargets(Player* player)
{
	...
	for (Group::MemberSlot const& slot : memList)
	{
		...
		if (!player || !player->isAlive() || (!player->GetSession() || !sPlayerBotMgr->IsPlayerBot(player->GetSession())))
			continue;
...
void BotUtility::ProcessGroupCombatMovement(Player* pCenterPlayer, BOTAI_WORKTYPE aiType)
{
	...
	Unit* pCenterUnit = (pCenterPlayer->GetSelectedUnit()) ? pCenterPlayer->GetSelectedUnit() : pCenterPlayer;
	G3D::Vector3 relativePos = (G3D::Vector3(pCenterUnit->GetPositionX(), pCenterUnit->GetPositionY(), pCenterUnit->GetPositionZ()) - G3D::Vector3(pCenterPlayer->GetPositionX(), pCenterPlayer->GetPositionY(), pCenterPlayer->GetPositionZ())) + G3D::Vector3(pCenterUnit->GetPositionX(), pCenterUnit->GetPositionY(), pCenterUnit->GetPositionZ());
...
bool BotUtility::FindFirstCollisionPosition(Unit* pTargetUnit, float range, Unit* pRefUnit, Position& outPos)
{
	...
	if (pTargetUnit->GetMap() != pMap)
		return false;
	//G3D::Vector3 refUnitV3 = G3D::Vector3(pRefUnit->GetPositionX(), pRefUnit->GetPositionY(), pRefUnit->GetPositionZ());
	//G3D::Vector3 tarUnitV3 = G3D::Vector3(pTargetUnit->GetPositionX(), pTargetUnit->GetPositionY(), pTargetUnit->GetPositionZ());
...
{
	float posGap = 0.2f;
	if ((G3D::Vector3(pos1.GetPositionX(), pos1.GetPositionY(), pos1.GetPositionZ()) - G3D::Vector3(pos2.GetPositionX(), pos2.GetPositionY(), pos2.GetPositionZ())).length() > posGap)
		return true;
...
bool BotAIFindNearLoot::DoFindLoot(uint32 diff, BotBGAIMovement* movement, uint32 downMountID)
{
	...
	if (!pGroup || pGroup->isBGGroup() || false /*AllGroupNotCombat*/)
	{
		if (!me->IsStandState())
			me->SetStandState(UNIT_STAND_STATE_STAND);
		m_HasLoot = false;
...
ObjectGuid BotAIRevivePlayer::SearchNeedRevive(uint32 diff)
{
	...
	if (!pGroup)
		return ObjectGuid::Empty;
            std::vector<ObjectGuid> needPlayers; /*GetGroupMemberFromNeedRevivePlayer disabled*/
...
void BotAIFlee::UpdateFleeMovementByPVE(Unit* pMaster, Unit* pRefUnit, BotBGAIMovement* pMovement)
{
	...
	{
		float fleeDistance = (G3D::Vector3(m_FleeTarget->GetPositionX(), m_FleeTarget->GetPositionY(), m_FleeTarget->GetPositionZ()) - G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ())).length();
...
void BotAIIDLEMovement::UpdateIDLEMovement(BotBGAIMovement* pMovement)
{
	...
	if (Moveing())
	{
		float idleDistance = (G3D::Vector3(m_IDLETarget->GetPositionX(), m_IDLETarget->GetPositionY(), m_IDLETarget->GetPositionZ()) - G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ())).length();
...
{
	if (me->isInCombat() || (!me->GetSession() || !sPlayerBotMgr->IsPlayerBot(me->GetSession())))
		return;
	WorldSession* pSession = me->GetSession();
	if (!me->IsInWorld() || !me/*IsSettingFinish*/ true || !pSession || pSession/*HasSchedules*/ false || pSession/*IsAccountBotSession*/ false)
	{
...
bool BotAICheckDuel::CheckDuel()
{
	...
	if (me->isInCombat() || !me->IsInWorld() || !me/*IsSettingFinish*/ true ||
...
bool BotAIMovetoUseGO::ProcessMovetoUseGO(BotBGAIMovement* pMovement)
{
	if (m_RiteSpellID)
	{
		...
		if (GameObject* go = me->GetGameObject(m_RiteSpellID))
		{
			...
			{
				Player* player = ObjectAccessor::FindPlayer(slot.Guid);
				if (!player || (!player->GetSession() || !sPlayerBotMgr->IsPlayerBot(player->GetSession())) || !player->isAlive() || me->GetMap() != player->GetMap() || !player->IsInWorld())
					continue;
...
void BotDoRangedAttackIfReady(Player* me)
{
    if (me->HasUnitState(UNIT_STATE_CASTING))
        return;
    ...
    if (ItemTemplate const* rangedTemplate = rangedItem ? rangedItem->GetTemplate() : nullptr)
    {
        switch (rangedTemplate->BasicData->SubclassID)
        {
            ...
            case ITEM_SUBCLASS_WEAPON_CROSSBOW:
                rangedAttackSpell = 75; // SPELL_SHOOT
                break;
            case ITEM_SUBCLASS_WEAPON_THROWN:
                rangedAttackSpell = 2764; // SPELL_THROW
                break;
            case ITEM_SUBCLASS_WEAPON_WAND:
                rangedAttackSpell = 5019; // SPELL_SHOOT_WAND
                break;
        }
    }
