...

{
...

	{
...

		{
			...
			if (!player || me->GetMap() != player->GetMap() || !player->IsInWorld() || (player->GetSession() && sPlayerBotMgr->IsPlayerBot(player->GetSession())))
...

	{
...

		{
			...
			if (pTarget)
				...
				pSession->HandlePetActionHelper(pPet, pPet->GetGUID(), 2, 7, pTarget->GetGUID(), Position());
...

{
...

	{
		...
		Position pos;
			me->GetFirstCollisionPosition(pos, BOTAI_RANGESPELL_DISTANCE, angle);
