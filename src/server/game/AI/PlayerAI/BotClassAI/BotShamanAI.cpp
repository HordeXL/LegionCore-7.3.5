...

{
	...
	m_BotTalentType = 0;// PlayerBotSetting::FindPlayerTalentType(me);
...

{
	...
	std::shuffle(friends.begin(), friends.end(), std::default_random_engine(time(NULL)));
...

{
...

	{
		...
		if (m_NonFearTotemTick > 0 || !ExistTotemByType(1, BOTAI_TOTEMRANGE))
	...
	if (ShamanTotem_AbsorbBuff && !ExistTotemByType(3, BOTAI_TOTEMRANGE))
...

	{
		...
		if (!ExistTotemByType(2, BOTAI_TOTEMRANGE))
...

	{
		...
		bool existFireTotem = ExistTotemByType(0, BOTAI_TOTEMRANGE);
...
bool BotShamanAI::ExistTotemByType(int totemType, float range)

{
	...
	else if (totemType == 1) // 1

		slotType = SUMMON_SLOT_TOTEM + 1;

	else if (totemType == 2) // 2

		slotType = SUMMON_SLOT_TOTEM + 2;

	else if (totemType == 3) // 3
