...
bool BotFieldAI::ProcessGroupInvite()
{
	...
	if (pSession/*HasSchedules*/ false || me->InBattleground())
	{
...
{
	if (me->GetSession()/*HasBGSchedule*/ false || me->InBattleground() || me->isUsingLfg())
		return true;
	return false;
}
...
SpellCastResult BotFieldAI::TryCastSpell(uint32 spellID, Unit* pTarget, bool force, bool dismount)
{
	...
	if (castResult != SpellCastResult::SPELL_CAST_OK)
	{
		...
		else if (castResult == SpellCastResult::SPELL_FAILED_BAD_TARGETS)
		{
			...
			if (pTarget && pTarget->ToPlayer() && (!pTarget->GetSession() || !sPlayerBotMgr->IsPlayerBot(pTarget->GetSession())) && !pTarget->IsPvP())
			{
