...

{
...

	{
...

		{
			...
			Position pos; me->GetFirstCollisionPosition(pos,me->GetDistance(targetPos) + range, me->GetRelativeAngle(&targetPos) + rndOffset);
...

{
	...
	//	G3D::Vector3 dir = (G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()) - G3D::Vector3(selectEnemy->GetPositionX(), selectEnemy->GetPositionY(), selectEnemy->GetPositionZ())).direction();

	//	Position targetPos = me->GetFirstCollisionPosition(BOTAI_SEARCH_RANGE * 0.5, selectEnemy->GetRelativeAngle(&me->GetPosition()));

	//	if ((G3D::Vector3(targetPos.GetPositionX(), targetPos.GetPositionY(), targetPos.GetPositionZ()) - G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ())).length() <= 5.0f)
...

{
	...
	case CLASS_PALADIN:
		...
		return (pTarget->GetTalentType() != 0);
	...
	case CLASS_SHAMAN:
		...
		return (pTarget->GetTalentType() == 1);
...

{
	...
	case CLASS_PRIEST:
		...
		return (pTarget->GetTalentType() != 2);
...

{
	...
	if ((!pTarget->ToPlayer()->GetSession() || !sPlayerBotMgr->IsPlayerBot(pTarget->ToPlayer()->GetSession())))
